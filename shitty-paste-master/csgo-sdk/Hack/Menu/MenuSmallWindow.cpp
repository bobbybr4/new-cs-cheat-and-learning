#include "MenuSmallWindow.h"

MSmallWindow::MSmallWindow(std::string name, Vector pos, Vector size, MBaseElement* parent, bool render_if_no_menu)
{
	this->position = pos;
	this->size = size;
	this->name = name;
...
void MSmallWindow::Draw()
{
	...
	if ((m_pMenu->MainWindow->InBounds(Vector(this->position.x, this->position.y, 0), Vector(this->size.x, dragyoffset, 0)) && m_pMenu->MainWindow->bMouse1pressed))
		this->bDragging = true;
	if (this->bDragging)
	{
