...
void MGroupbox::Draw()
{
	...
	if (inside)
	{
		...
		g_pRender->BorderedBox(position.x, position.y, size.x, size.y, D3DCOLOR_ARGB(alpha, 20, 20, 20));
		//font size
	...
	else
	{
		...
		this->position.x = m_pMenu->MainWindow->position.x + 55; //not +10 because
		this->position.y += m_pMenu->MainWindow->dragYoffset + 15;

		int y_minus = this->y_blocks == 2 ? 20 : 30;

		this->size.x = (m_pMenu->MainWindow->size.x - 80) / 3 * x_blocks; //left, right and middle shit offsets
		this->size.y = (m_pMenu->MainWindow->size.y - y_minus - (m_pMenu->MainWindow->dragYoffset)) / 2 * y_blocks;

		int x = this->position.x + ((m_pMenu->MainWindow->size.x - 80) / 3) * (this->x_offset - 1);
		int y = this->position.y + (10 + (m_pMenu->MainWindow->size.y - y_minus - m_pMenu->MainWindow->dragYoffset) / 2) * (this->y_offset - 1);

		g_pRender->BorderedBox(x, y, this->size.x - 30, this->size.y, D3DCOLOR_ARGB(alpha, 20, 20, 20));

		int name_x = x + (this->size.x - 30) / 2;
		int name_y = this->position.y + (10 + (m_pMenu->MainWindow->size.y - y_minus - m_pMenu->MainWindow->dragYoffset) / 2) * (this->y_offset - 1);
		name_y -= 12 / 2; //font size
		g_pRender->Text((char*)this->caption.c_str(), name_x, name_y, centered, g_pRender->Fonts.menu_control, true, WHITE(alpha), BLACK(alpha));

		this->position.x += ((m_pMenu->MainWindow->size.x - 80) / 3) * (this->x_offset - 1);
