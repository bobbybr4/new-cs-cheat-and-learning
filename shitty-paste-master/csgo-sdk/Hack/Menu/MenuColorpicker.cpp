...
DWORD ModOpacity(DWORD color, float opacity) {
...
void MColorPicker::Draw()
{
	...
	this->position.x += 10;
	this->position.y += 10;
	this->size = Vector(parent->size.x - 20, 10, 0);
	...
	DWORD modcol = ModOpacity(this->color, (alpha / 255));
	...
	g_pRender->FilledBoxOutlined(draw_x, position.y + 1, 20, 10, modcol, BLACK(alpha));

	if (m_pMenu->MainWindow->Clicked(Vector(draw_x, position.y + 1, 0), Vector(20, 10, 0)))
		is_open = true;

	if (is_open)
	{

		DrawColorPicker(draw_x, position.y + 15, g_pRender->device);
		g_pRender->BorderedBox(draw_x, position.y + 15, 256, 256, BLACK(alpha));

		if (m_pMenu->MainWindow->bMouse1pressed && m_pMenu->MainWindow->InBounds(Vector(draw_x, position.y + 15, 0), Vector(256, 256, 0)))
		{
			this->color = GetPickedColor(m_pMenu->MainWindow->Cur.x - draw_x, m_pMenu->MainWindow->Cur.y - position.y + 15);
		}

		if (m_pMenu->MainWindow->bMouse1released && !m_pMenu->MainWindow->InBounds(Vector(draw_x, position.y, 0), Vector(256, 256 + 25, 0)) || !m_pMenu->MainWindow->visible)
			is_open = false;
	}
}