...
void __fastcall Hooks::PaintTraverse(void* ecx, void* edx, unsigned int panel, bool forceRepaint, bool allowForce) {
	if (g_pSettings->VisualsRemovalScope->Checked && G::LocalPlayer && G::LocalPlayer->GetWeapon()) {
	...
	if (g_pSettings->VisualsRemovalScope->Checked && (G::LocalPlayer && G::LocalPlayer->GetWeapon() && G::LocalPlayer->GetWeapon()->IsSniper() && G::LocalPlayer->GetWeapon()->GetZoomLevel() > 0)) {
		int screenX, screenY;
